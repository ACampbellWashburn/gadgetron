#include "gpuGenericSensePrepGadgetRegrid.h"
...
namespace Gadgetron{
  ...
  gpuGenericSensePrepGadgetRegrid::gpuGenericSensePrepGadgetRegrid()
    : slices_(-1)
    , sets_(-1)
    , device_number_(-1)
  ...
  int gpuGenericSensePrepGadgetRegrid::process_config(ACE_Message_Block* mb)
  {
    // Get configuration values from config file
    //
    GDEBUG("ACW process_config");
  ...
  int gpuGenericSensePrepGadgetRegrid::
          ...
          GadgetContainerMessage< hoNDArray<float> > *m3)                   // traj/dcw
  {
    ...
    if( output_timing_ )
      process_timer = boost::shared_ptr<GPUTimer>( new GPUTimer("gpuGenericSensePrepGadgetRegrid::process()") );
    ...
    if( is_last_readout_in_frame || 
      ...
      GDEBUG("ACW process 4 set = %i\n",header->getObjectPtr()->set);
    }
  ...
  gpuGenericSensePrepGadgetRegrid::extract_samples_from_queue ( ACE_Message_Queue<ACE_MT_SYNCH> *queue, 
  ...
  {    
    ...
    for (unsigned int p=0; p<readouts_buffered; p++) {
      ...
      if (queue->dequeue_head(mbq) < 0) {
        ...
        throw std::runtime_error("gpuGenericSensePrepGadgetRegrid::extract_samples_from_queue: dequeing failed");	
      }
      ...
      if (!daq) {
        ...
        throw std::runtime_error("gpuGenericSensePrepGadgetRegrid::extract_samples_from_queue: failed to interpret data");	
      }
	...
	//ACW
	//for(int i=0;i<samples_per_readout_;i++){
	  //GDEBUG("ACW r_ptr = %f\n",std::abs(r_ptr[i]));
	//}
  ...
  gpuGenericSensePrepGadgetRegrid::extract_trajectory_from_queue ( ACE_Message_Queue<ACE_MT_SYNCH> *queue, 
                                                             bool sliding_window, unsigned int set, unsigned int slice )
  {    
    if(!queue) {
      GDEBUG("Illegal queue pointer, cannot extract trajectory\n");
      throw std::runtime_error("gpuGenericSensePrepGadgetRegrid::extract_trajectory_from_queue: illegal queue pointer");	
    }

    if(queue->message_count()==0) {
      GDEBUG("Empty queue, cannot extract trajectory\n");
      throw std::runtime_error("gpuGenericSensePrepGadgetRegrid::extract_trajectory_from_queue: empty queue");	
    }
    ...
    for (unsigned int p=0; p<readouts_buffered; p++) {      
      ...
      if (queue->dequeue_head(mbq) < 0) {
        GDEBUG("Message dequeue failed\n");
        throw std::runtime_error("gpuGenericSensePrepGadgetRegid::extract_trajectory_from_queue: dequeing failed");	
      }
      ...
      if (!daq) {
        GDEBUG("Unable to interpret data on message queue\n");
        throw std::runtime_error("gpuGenericSensePrepGadgetRegrid::extract_trajectory_from_queue: failed to interpret data");	
      }
  ...
  void gpuGenericSensePrepGadgetRegrid::extract_trajectory_and_dcw_from_queue
  ...
  gpuGenericSensePrepGadgetRegrid::duplicate_array( GadgetContainerMessage< hoNDArray<T> > *array )
  {
  ...
  void gpuGenericSensePrepGadgetRegrid::reconfigure(unsigned int set, unsigned int slice)
  {    
    unsigned int idx = set*slices_+slice;
    GDEBUG("ACW idx = %i, ACW readoutsperframe = %i\n",idx, readouts_per_frame_[idx]); 
    GDEBUG("ACW Reconfigure Sets, slices, slice = %d, %d, %d\n",sets_,slices_,slice);
  ...
  GADGET_FACTORY_DECLARE(gpuGenericSensePrepGadgetRegrid)
}
